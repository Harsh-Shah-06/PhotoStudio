...
if (isset($_REQUEST['btn_submit'])) {
    ...
    for ($i = 1; $i <= $no; $i++) {
        ...
        $path = "img/main/" . $newfilename;
        move_uploaded_file($tmp, $path);

        $in = "insert into maingallery values('','$newfilename','$a')";
        mysqli_query($con, $in);
    }
}
...
if (isset($_REQUEST['id'])) {
    ...
    $update = "update maingallery set Status = '$sta' where Id = '$id'";
    echo $update . "<br />";
    mysqli_query($con, $update) or die(mysqli_errno($con));
    echo '<script>window.location.href="mgallery.php"</script>';
}
...
if (isset($_REQUEST['btn_update'])) {
    ...
    if ($name != '') {
        $path = "img/main/" . $newfilename;
        move_uploaded_file($tmp, $path) or die("hello");


        $up = "update maingallery set Path='$newfilename' where Id = '$txt_id'";
        mysqli_query($con, $up);
        echo '<script>window.location.href="mgallery.php"</script>';
    }
}
    ...
    <div class="row-fluid">
        ...
        <div class="block">
            <div class="navbar navbar-inner block-header">
                <div class="muted pull-left">Main Gallery</div>
                                    ...
                                    <div class="span12">
                                        ...
                                        <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
                                            ...
                                            <tbody>
                                                <?php
                                                $i = 1;
                                                $sel = "select * from maingallery";
                                                $res = mysqli_query($con, $sel);
                                                while ($row = mysqli_fetch_array($res)) {
                                                    ?>
                                                    <tr align="center">	
                                                        <td><?php echo $i++ ?></td>
                                                        <td><img src="img/main/<?php echo $row['Path'] ?>" alt=""  height="200px" width="200px" ></td>        
                                                        <td>
                                                            <?php
                                                            if ($row['Status'] == 'Active') {
                                                                ?>
                                                            <a href="mgallery.php?id=<?php echo $row['Id'] ?>&status=<?php echo $row['Status'] ?>" onClick="return confirm('Are U Sure want to Deactive ?')"><input type="button" class="btn btn-success" value="<?php echo $row['Status'] ?>"></a>
                                                                <?php
                                                            } else {
                                                                ?>
                                                            <a href="mgallery.php?id=<?php echo $row['Id'] ?>&status=<?php echo $row['Status'] ?>" onClick="return confirm('Are U Sure want to Active ?')"><input type="button" class="btn btn-danger" value="<?php echo $row['Status'] ?>"></a>
                                                                <?php
                                                            }
                                                            ?>
